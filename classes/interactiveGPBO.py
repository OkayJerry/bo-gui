...
class simpleNN(torch.nn.Module):
    def __init__(self, n_decision, n_env, 
        ...
        self.seq = [torch.nn.Linear(n_decision+n_env,list_of_nodes[0]),activation]
        for i in range(1,len(list_of_nodes)-1):
            self.seq += [torch.nn.Linear(list_of_nodes[i-1],list_of_nodes[i]), activation]
...
class prior_mean_model_wrapper(torch.nn.Module):
    def __init__(self,prior_mean_model,decision_transformer,environmental_parameters):
        ...
        if environmental_parameters is None:
            ...
            self.env = torch.tensor(environmental_parameters,dtype=torch.float32)
...
class InteractiveGPBO:
    ...
    def step(self,x=None,y=None,
        ...
        yt = torch.tensor(-y,dtype=torch.float32)  # sign flip for minimization (instead of maximization)
        ...
        if self.avoid_bounds_corners and self.prior_mean_model is None:
            ...
            corner_grid = np.array(list(list(x.flat) for x in corner_grid),np.float32).T
        ...
        xt1, yt1acqu = optimize_acqf(acqu, 
                                     bounds=torch.stack([-torch.ones(self.dim), torch.ones(self.dim)]), 
        ...
        if remember_gp or write_gp_on_grid:
            ...
            if write_gp_on_grid:
                self.write_gp_on_grid(self.epoch,grid_ponits_each_dim=grid_ponits_each_dim)
        else:
    ...
    def update_exploration(self,x1,y1,x=None,y=None):
        ...
        self.history['y_min'] = np.array([min(y[:i+1]) for i in range(len(y))]).flatten()
    ...
    def write_gp_on_grid(self,epoch=None,path="./log/",tag="",grid_ponits_each_dim=25):
        if grid_ponits_each_dim**self.dim*self.dim > 2e6:
            print("Due to high-dimensionality and large number of grid point, GP data on grid will not be created nor saved into file. Try again with lower number of 'grid_ponits_each_dim'")
            return
        if path[-1]!="/":
            path += "/"
        if not os.path.isdir(path):
        ...
        for i in iiter:
            if self.history['gp'][i-1] is None:
                print("gp at epoch " + str(i)+" is not saved in memeory. GP data on grid will not be create and saved into file")
                continue
            linegrid = [np.linspace(self.history['bounds'][i-1][d,0],
                                    self.history['bounds'][i-1][d,1],
                                    grid_ponits_each_dim,dtype=np.float32) for d in range(self.dim)]
            x_grid = np.meshgrid(*linegrid)
            x_grid = np.array(list(list(x.flat) for x in x_grid)).T.astype(np.float32)
            x_grid_t = torch.tensor(self.normalize(x_grid,self.history['bounds'][i-1]),dtype=torch.float32)
            with torch.no_grad():
                # decide batch size in multiple of "grid_ponits_each_dim"
                batch_size = 1
                ...
                for b in range(n_batch):
                    ...
                    if "knowledge_gradient" in str(type(self.history['acquisition'][i-1])):
                        ...
                        y_acq.append(acq(x_batch.view(-1,1,self.dim)).detach().numpy())
                    else:
                        y_acq.append(self.history['acquisition'][i-1](x_batch.view(-1,1,self.dim)).detach().numpy())
            data = {'x_grid':x_grid,
            ...
            pickle.dump(data,open(path+tag+"GPlog_epoch"+str(i)+".pickle","wb"))
    ...
    def plot_best(self,ax=None):
        y_best_hist = [np.min(self.history['y'][-1][:i+1]) for i in range(len(self.history['y'][-1]))]
    ...
    def plot_aquisition_2D_projection(self,epoch=None,
        ...
        '''
        if grid_ponits_each_dim**(self.dim-2)*(self.dim-2) > 2e6 and (project_minimum or project_mean):
            print("Due to high-dimensionality and large number of grid point, plot_aquisition_2D_projection with 'project_minimum=True' or 'project_mean=True' cannot proceed. Try again with lower number of 'grid_ponits_each_dim' or use 'fixed_values_for_each_dim'")
            return
        if epoch is None:
            epoch = self.epoch

        if hasattr(self.history['acquisition'][epoch-1],'beta_prime'):
            beta = (self.history['acquisition'][epoch-1].beta_prime*2/np.pi)**2
        else:
            beta = 4
        ...
        if self.dim > 2:
            ...
            n_batch = int(grid_ponits_each_dim**(self.dim-2)/batch_size)
            
        
        linegrid = np.linspace(0,1,grid_ponits_each_dim)
        x_grid = np.zeros((grid_ponits_each_dim*grid_ponits_each_dim,self.dim))
        ...
        for i in progressbar(range(grid_ponits_each_dim)):
            ...
            for j in range(grid_ponits_each_dim):
                ...
                if (project_minimum or project_mean) and self.dim > 2:
                    ...
                    inner_grid = np.array(list(list(x.flat) for x in inner_grid),np.float32).T
                    inner_grid_n = torch.tensor(self.normalize(inner_grid,self.history['bounds'][epoch-1]),dtype=torch.float32)   
        ...
        if (not (project_minimum or project_mean)) or self.dim==2:
            ...
            x_grid_n = torch.tensor(self.normalize(x_grid,self.history['bounds'][epoch-1]),dtype=torch.float32)
        ...
        ax.tricontourf(x_grid[:,dim_xaxis], x_grid[:,dim_yaxis], y_grid, levels=64, cmap="viridis");
        if self.dim > 2:
            tag = "projected "
        else:
            tag = ""
        ax.scatter(x [:,dim_xaxis],x [:,dim_yaxis],c="b",label = tag+"training data")
        ax.scatter(x1[:,dim_xaxis],x1[:,dim_yaxis],c="r",label = tag+"candidate")    
        # import inspect
        # print(inspect.stack()[0][3])
        # print(inspect.stack()[1][3])
